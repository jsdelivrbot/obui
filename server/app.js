...
if (!uaaIsConfigured) { // no restrictions
        ...
        app.get('/api/asset-list/*', passport.authenticate('main', { noredirect: true }),
                // if calling a secure microservice, you can use this middleware to add a client token.
                proxy.addClientTokenMiddleware,
                proxy.customProxyMiddleware('/api', 'https://ob-datasource.run.aws-jp01-pr.ice.predix.io/services/datagrid')
        );

        app.post('/api/v1/datapoints/*', passport.authenticate('main', { noredirect: true }),
                // if calling a secure microservice, you can use this middleware to add a client token.
                proxy.addClientTokenMiddleware,
                proxy.customProxyMiddleware('/api', 'https://time-series-store-predix.run.aws-jp01-pr.ice.predix.io')
        );
        app.post('/api/v1/datapoints', passport.authenticate('main', { noredirect: true }),
                // if calling a secure microservice, you can use this middleware to add a client token.
                proxy.addClientTokenMiddleware,
